...
```bash
python ./.cstl/scripts/get_context.py --mode packages   # list packages / layers
```
...
```bash
# Task lifecycle
python ./.cstl/scripts/task.py create "<title>" [--slug <name>] [--parent <dir>]
python ./.cstl/scripts/task.py dashboard             # show Task Dashboard without mutating state
python ./.cstl/scripts/task.py select <name>         # select task for this live session
python ./.cstl/scripts/task.py selected --source     # show selected task and source
python ./.cstl/scripts/task.py start-execution <name> --check
python ./.cstl/scripts/task.py start-execution <name> --approved
python ./.cstl/scripts/task.py exit                  # clear selected task without changing status
python ./.cstl/scripts/task.py archive <name>        # move to archive/{year-month}/
python ./.cstl/scripts/task.py list [--mine] [--status <s>]
python ./.cstl/scripts/task.py list-archive
python ./.cstl/scripts/task.py add-subtask <parent> <child>
python ./.cstl/scripts/task.py set-child-state <parent> <child> review --evidence verify.md
python ./.cstl/scripts/task.py prepare-child-worktree <parent> <child> --branch <child-branch>
python ./.cstl/scripts/task.py integrate-child <parent> <child> accepted --evidence handoff.md --ref <child-ref>

# Code-spec context (injected into implement/check agents via JSONL).
# `implement.jsonl` / `check.jsonl` are seeded on `task create` for sub-agent-capable
# platforms; the AI curates real spec + research entries during planning when needed.
python ./.cstl/scripts/task.py add-context <name> <action> <file> <reason>
python ./.cstl/scripts/task.py list-context <name> [action]
python ./.cstl/scripts/task.py validate <name>

# Task metadata
python ./.cstl/scripts/task.py set-branch <name> <branch>
python ./.cstl/scripts/task.py set-base-branch <name> <branch>    # PR target
python ./.cstl/scripts/task.py set-scope <name> <scope>

# Hierarchy (parent/child)
python ./.cstl/scripts/task.py add-subtask <parent> <child>
python ./.cstl/scripts/task.py remove-subtask <parent> <child>

# PR creation
python ./.cstl/scripts/task.py create-pr [name] [--dry-run]
```

> Run `python ./.cstl/scripts/task.py --help` to see the authoritative, up-to-date list.
...
```bash
python ./.cstl/scripts/add_session.py --title "Title" --commit "hash" --summary "Summary"
```

### Context Script

```bash
python ./.cstl/scripts/get_context.py                            # full session runtime
python ./.cstl/scripts/get_context.py --mode packages            # available packages + spec layers
python ./.cstl/scripts/get_context.py --mode phase --step <X.Y>  # detailed guide for a workflow step
python ./.cstl/scripts/get_context.py --mode retrieval-pack --json --input <evidence.json>  # score collected evidence (not default --json)
```
...
python ./.cstl/scripts/task.py review-child <parent-task> <child-task> --check --decision accept --ref <child-ref>
...
```bash
python ./.cstl/scripts/get_context.py --mode phase --step <step>
# e.g. python ./.cstl/scripts/get_context.py --mode phase --step 1.1
```
...
```bash
python ./.cstl/scripts/task.py create "<task title>" --slug <name>
```
...
Skip when the user has already explicitly selected an appropriate task with `python ./.cstl/scripts/task.py select <task>`.
...
- Use `python ./.cstl/scripts/search_artifacts.py --query "<topic>" --json` to find durable Trellis specs, prior tasks, research, verification notes, and workspace journals before re-discovering framework context.
...
```bash
python ./.cstl/scripts/task.py add-context "$TASK_DIR" implement "<path>" "<reason>"
python ./.cstl/scripts/task.py add-context "$TASK_DIR" check "<path>" "<reason>"
```
...
- `python ./.cstl/scripts/search_artifacts.py --query "<topic>" --json` for durable Trellis specs, prior task artifacts, research, verification notes, and journals.
...
`python ./.cstl/scripts/get_context.py --mode retrieval-pack --json --input <path-to-evidence.json>`
