...
| Command | Role | Exit |
| --- | --- | --- |
| `python ./.cstl/scripts/pool.py validate` | Four-pack / required keys / status / unique id / dangling links / bidirectional consistency | error-level → 1 |
| `python ./.cstl/scripts/pool.py plan-check` | plan.md referenced ids exist (ghost id → error) | same |
| `python ./.cstl/scripts/pool.py link <item-id> <task-ref>` | Bidirectional link (idempotent) | missing → 1 |
| `python ./.cstl/scripts/pool.py unlink <item-id> <task-ref>` | Bidirectional unlink | item missing → 1 |
| `python ./.cstl/scripts/pool.py show <item-id>` | Print item + linked task status summary | missing → 1 |
