...
- 🔤 网易云没有逐字歌词时，依次回落到社区歌词库 [AMLL TTML DataBase](https://github.com/amll-dev/amll-ttml-db)（CC0-1.0）和 QQ 音乐 QRC（仅桌面端，可在设置中关闭）
- 📻 支持私人 FM / 每日推荐歌曲
- 🎯 播放队列页展示相似歌曲与相似歌单
- 🕘 音乐库支持「最近播放」（单曲 / 歌单 / 专辑）
- 🚫🤝 无任何社交功能
...
- ☁️ 支持音乐云盘
- ⌨️ 自定义快捷键和全局快捷键
- 🎮 游戏模式：一键把界面降级成纯播放内核，把带宽 / CPU / 磁盘让给游戏（见下文）
- 🖥️ 桌面歌词：透明置顶窗口，浮在其他应用之上显示逐字歌词（仅桌面端，见下文）
- 🎧 支持 Mpris
- 🛠 更多特性开发中

### 🎮 游戏模式

打游戏时播放器的整套界面都在后台白烧资源：常驻内存的页面缓存、每帧合成的毛玻璃背景、每首歌几百 KB 用不上的封面预下载、持续的元数据落盘——这些跟「把当前这首歌放出声」没有任何关系。

游戏模式开启后：

- 界面只剩一块最小播放面板（封面、歌名、进度条、上一首/播放/下一首、音量、喜欢、循环模式），其余页面、歌词页、毛玻璃与弹窗全部卸载
- 不再预下载各尺寸封面，不再拉取音乐库数据，不加载统计脚本
- 不再往 IndexedDB 写歌曲详情 / 歌词 / 专辑 / 歌单等元数据缓存
- 播放进度写盘从每秒 1 次降到每 10 秒 1 次，设置持久化改为 10 秒防抖
- 下一首的预下载推迟到当前歌曲播过半再触发，避开切歌时的带宽尖峰
...
- 设置页的「游戏模式」开关
- 全局快捷键 `Alt+Ctrl+G`（游戏在前台时也有效，可在设置中修改）
- 托盘菜单的「游戏模式」

> 目前只做手动开关，不做「检测到全屏游戏自动进入」——Electron 没有可靠的跨平台方式判断别的窗口是否在全屏游戏，现有方案会把浏览器全屏、PPT 放映一并误报。

### 🖥️ 桌面歌词

一个透明、无边框、置顶的独立窗口，浮在其他应用之上显示当前这一句，逐字高亮与歌词页一致。

- 双行显示：当前句（逐字扫过）+ 译文（可关）+ 下一句（暗色）
- 高亮色跟随封面主色；没歌词、纯音乐、歌词还在路上时显示「歌名 — 歌手」
- 未锁定时拖歌词本体即可挪窗口，鼠标悬停顶部浮出工具条（上一首 / 播放 / 下一首 / 字号 / 锁定 / 关闭），左右边缘可拖动改宽度
- 锁定后整个窗口鼠标穿透，不挡下面的操作；悬停顶部仍会浮出工具条，可以点解锁
- 与游戏模式互不影响，打游戏时照样显示（那时界面被替换成纯播放面板，只能用全局快捷键开关）

开关（三个入口任选）：

- 设置页的「桌面歌词」开关
- 播放栏右侧的桌面歌词按钮
- 全局快捷键 `Alt+Ctrl+D` 显示/隐藏、`Alt+Ctrl+K` 锁定/解锁（均可在设置中修改）

> **盖不住独占全屏的游戏**。Electron 的置顶层级最高只到 `screen-saver`，独占全屏是显示模式的问题而不是窗口层级的问题。窗口化 / 无边框窗口化的游戏没有这个限制。
>
> 窗口不能用原生边框缩放：Electron 官方在 transparent 窗口的限制里写明「Transparent windows are not resizable」，所以宽度改由页面内的把手实现。
